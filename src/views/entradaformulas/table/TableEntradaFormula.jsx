...
import { EntradasColumns } from './DataEntradaFormula';
...
function TableEntradaFormula({ searchTerm, setSearchTerm }) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          getEntradas(res.data);
          Swal.fire('Eliminado', 'La entrada de formula ha sido eliminado correctamente', 'success');
        } catch (error) {
          Swal.fire('Error', 'Ha ocurrido un error al eliminar la entrada de formula', 'error');
        }
      }
    });
  };
  ...
  const hasUpdatePermission = sessionStorage.getItem("auth_permisos")?.includes("entrada_formulas.update");
  const hasDestroyPermission = sessionStorage.getItem("auth_permisos")?.includes("entrada_formulas.destroy");
        ...
        return (
          ...
          <Stack direction="row" spacing={2}>
            ...
            {hasDestroyPermission && (
              <Button variant="outlined" onClick={() => deleteEntradas(params.id)} startIcon={<DeleteIcon />} color="warning">
                Eliminar
              </Button>
            )}
          </Stack>
          ...
          <DataGrid
            ...
            rows={entradas.filter((row) => {
              ...
              return (
                ...
                (row.fecha_entrada?.toLowerCase() || '').includes(searchTerm.toLowerCase())
              );
            })}
            columns={EntradasColumns.concat(actionColumn)}
            pageSize={10}
...
TableEntradaFormula.propTypes = {
  searchTerm: PropTypes.string.isRequired,
  setSearchTerm: PropTypes.func.isRequired,
};

export default TableEntradaFormula;