...
import { PacientesColumns } from './DataPaciente';
...
function TablePaciente({ searchTerm, setSearchTerm }) {
  ...
  const memoizedPacientes = useMemo(() => {
    return paciente;
  }, [paciente]);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          getPaciente(res.data);
          Swal.fire('Eliminado', 'El paciente ha sido eliminado correctamente', 'success');
        } catch (error) {
          Swal.fire('Error', 'Ha ocurrido un error al eliminar el paciente', 'error');
        }
      }
    });
  };
  ...
  const hasUpdatePermission = sessionStorage.getItem("auth_permisos")?.includes("pacientes.update");
  const hasDestroyPermission = sessionStorage.getItem("auth_permisos")?.includes("pacientes.destroy");
        ...
        return (
          ...
          <Stack direction="row" spacing={2}>
            ...
            {hasDestroyPermission && (
              <Button
                variant="outlined"
                onClick={() => deletePaciente(params.id)}
    ...
    <ThemeProvider theme={theme}>
      <div className={classes.tableContainer}>
        ...
        >
          {/* <DataGrid
            rows={paciente}
            columns={PacientesColumns.concat(actionColumn)}
            pageSize={10}
            components={{
              Toolbar: GridToolbar,
            }}
          ...
          <DataGrid
            ...
            rows={paciente.filter((row) => {
              return (
                row.dni_paciente.toLowerCase().includes(searchTerm.toLowerCase()) ||
                row.paciente.toLowerCase().includes(searchTerm.toLowerCase()) ||
                row.hospital?.desc_red.toLowerCase().includes(searchTerm.toLowerCase()) ||
                row.estacion?.desc_estacion.toLowerCase().includes(searchTerm.toLowerCase())
              );
            })}
            columns={PacientesColumns.concat(actionColumn)}
            ...
            initialState={{
              ...memoizedPacientes.initialState,
