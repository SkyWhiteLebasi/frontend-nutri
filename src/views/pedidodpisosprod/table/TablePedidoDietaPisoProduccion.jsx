...
import { PedidodPisosColumns } from './DataPedidoDietaPisoProduccion';
...
function TablePedidoDietaPisoProduccion({ searchTerm, setSearchTerm }) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire('Eliminado', 'El pedido segun el piso ha sido eliminado correctamente', 'success');
        } catch (error) {
          Swal.fire('Error', 'Ha ocurrido un error al eliminar el pedido segun el piso', 'error');
        }
      }
    });
  };
  ...
  const hasUpdatePermission = sessionStorage.getItem("auth_permisos")?.includes("pedidodpisosproduccion.update");
  const hasDestroyPermission = sessionStorage.getItem("auth_permisos")?.includes("pedidodpisosproduccion.destroy");
    ...
    <ThemeProvider theme={theme}>
      <div className={classes.tableContainer}>
        ...
        >
          {/* <DataGrid
            rows={pedidos}
            columns={PedidodPisosColumns.concat(actionColumn)}
            pageSize={10}
            components={{
              Toolbar: GridToolbar,
            }}
...
TablePedidoDietaPisoProduccion.propTypes = {
  searchTerm: PropTypes.string.isRequired,
  setSearchTerm: PropTypes.func.isRequired,
};
export default TablePedidoDietaPisoProduccion;