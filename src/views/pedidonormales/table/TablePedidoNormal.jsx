...
import { PedidosColumns } from './DataPedidoNormal';
...
function TablePedidoNormal({ searchTerm, setSearchTerm }) {

  const [pedidos, setPedidoNormal] = useState([])
  const classes = useStyles();

  const getPedidoNormal = async () => {
    try {
      const response = await axios.get(URI, {
        params: { searchTerm }, // Envía el término de búsqueda como parámetro en la solicitud
      });
      setPedidoNormal(response.data);
    } catch (e) {
      // Manejar errores
    }
  };
  ...
  const deletePedidoNormal = async (id) => {
    Swal.fire({
      title: '¿Estás seguro?',
  ...
  const hasUpdatePermission = sessionStorage.getItem("auth_permisos")?.includes("pedidosnormales.update");
  const hasDestroyPermission = sessionStorage.getItem("auth_permisos")?.includes("pedidosnormales.destroy");
        ...
        return (
          ...
          <Stack direction="row" spacing={2}>
            ...
            {hasDestroyPermission && (
              <Button variant="outlined" onClick={() => deletePedidoNormal(params.id)} startIcon={<DeleteIcon />} color="warning">
                Eliminar
              </Button>
            )}
          </Stack>
...
TablePedidoNormal.propTypes = {
  searchTerm: PropTypes.string.isRequired,
  setSearchTerm: PropTypes.func.isRequired,
};
export default TablePedidoNormal;