...
const Form = () => {
  ...
  const [detallesPaciente, setDetallePaciente] = useState("");
  ...
  const getPedido = async () => {
    try {
      ...
      setPacienteId(response.data.paciente_id);
    ...
    try {
      ...
      if (id) {
        const response = await axios.put(`${URI}${id}`, {
          ...
          // planillon_dieta_id: preparacionesPlanillon.map((item) => ({
          //     id: item.id || item.id_2,
          //     obs_preparacion: item.obs_preparacion,
          // }))
        });
  ...
  return (
    <Card>
      <ArgonBox p={1} mb={1} textAlign="center">
        <CardHeader title={id ? "ACTUALIZAR PEDIDO DIETA" : "REGISTRAR PEDIDO DIETA"} />
      </ArgonBox>
        ...
        <ArgonBox component="form" role="form" onSubmit={handleSubmit_1}>
              ...
              {/* <Grid item xs={6}>
                            <Item>
                                ...
                                <ArgonInput
                                    type="number"
                                    value={cantPersonal}
                                    onChange={(event) => setCantPersonal(event.target.value)}
                                />
                            </Item>
                        </Grid> */}
              ...
              <Grid item xs={6}>
                <Item>
                  ...
                  <Autocomplete
                    value={
                      pacienteId
                        ? pacientes.find((option) => {
                            return pacienteId === option.id;
                          }) ?? null
                        : null
                    }
                    onChange={(event, newValue) => {
                      setPacienteId(newValue ? newValue.id : null);
                      if (newValue) {
                        setDetallePaciente(
                          " DNI DEL PACIENTE: " +
                            newValue.dni_paciente +
                            " NOMBRE PACIENTE:  " +
                            newValue.paciente +
                            "  EDAD: " +
                            newValue.edad +
                            ...
                            newValue.cama +
                            "  ESTACION: " +
                            newValue.estacion?.desc_estacion +
                            "  HOSPITAL: " +
                            newValue.hospital?.desc_red
                        );
                      } else {
                        setDetallePaciente("");
                      }
                      setErrorList({ ...errorList, paciente_id: "" });
                    }}
                    disablePortal
                    sx={{ width: "100%", height: "100%" }}
                    disableListWrap
                    ...
                    ListboxComponent={ListboxComponent}
                    options={pacientes}
                    getOptionLabel={(option) => option.dni_paciente + " " + option.paciente}
                    renderInput={(params) => <TextField {...params} />}
                    renderOption={(props, option, state) => [
                      props,
                      option.dni_paciente + " " + option.paciente,
                      state.index,
                    ]}
                    // renderOption={(props, option, state) => [props,  , option.id + " " + option.nombre_ambiente]}
                    renderGroup={(params) => params}
                ...
                <Item>
                  ...
                  <FormControl variant="standard" sx={{ m: 1, minWidth: 430 }}>
                    <TextareaAutosize
                      ...
                      // multiline
                      rows={6}
                      value={detallesPaciente}
                      onChange={(event) => {
                        setDetallePaciente(event.target.value);
                        setErrorList({ ...errorList, detallesPaciente: "" });
                      }}
                    />
                  </FormControl>
                </Item>
                {errorList.detallesPaciente && (
                  <span style={{ color: "red", fontSize: "14px", marginTop: "5px" }}>
                    {errorList.detallesPaciente}
                  </span>
                )}
          ...
          <ArgonBox display="flex" justifyContent="center" mt={4} mb={2}>
            ...
            <CustomButton
              ...
              fullWidth
            >
              <Link to={"/pedidodietas"} style={{ color: "white" }}>
                Cancelar
              </Link>
            </CustomButton>
          </ArgonBox>
