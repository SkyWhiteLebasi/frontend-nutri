...
import { PlanillonColums } from './DataPlanillon';
...
function TablePlanillonNormal({ searchTerm, setSearchTerm }) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    getOrdenes(res.data);
                    Swal.fire('Eliminado', 'El planillon seleccionado ha sido eliminado correctamente', 'success');
                } catch (error) {
                    Swal.fire('Error', 'Ha ocurrido un error al eliminar el planillon seleccionada', 'error');
                }
            }
        });
    };
    ...
    const hasUpdatePermission = sessionStorage.getItem("auth_permisos")?.includes("planillones.update");
    const hasDestroyPermission = sessionStorage.getItem("auth_permisos")?.includes("planillones.destroy");
                ...
                return (
                    ...
                    <Stack direction="row" spacing={2}>
                        ...
                        {hasDestroyPermission && (
                            <Button variant="outlined" onClick={() => deleteOrden(params.id)} startIcon={<DeleteIcon />} color="warning">
                                Eliminar
                            </Button>
                        )}
                    </Stack>
        ...
        <ThemeProvider theme={theme}>
            <div className={classes.tableContainer}>
                ...
                >
                    ...
                    <DataGrid
                        {...memoizedPlanillon}
                        rows={ordenes.filter((row) => {
                            // Filtra las filas basadas en el término de búsqueda
                            const nroSemana = typeof row.programacion?.nro_semana === 'string' ? row.programacion?.nro_semana : String(row.programacion?.nro_semana);
                            return (
...
TablePlanillonNormal.propTypes = {
    searchTerm: PropTypes.string.isRequired,
    setSearchTerm: PropTypes.func.isRequired,
};
export default TablePlanillonNormal;