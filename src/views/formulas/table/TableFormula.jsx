...
import { FormulasColumns } from './DataFormula';
...
function TableFormula({ searchTerm, setSearchTerm }) {
  ...
  const memoizedFormulas = useMemo(() => {
    return formula;
  }, [formula]);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          getFormula(res.data);
          Swal.fire('Eliminado', 'La formula ha sido eliminado correctamente', 'success');
        } catch (error) {
          Swal.fire('Error', 'Ha ocurrido un error al eliminar la formula seleccionada', 'error');
        }
      }
    });
  };
  ...
  const hasUpdatePermission = sessionStorage.getItem("auth_permisos")?.includes("formulas.update");
  const hasDestroyPermission = sessionStorage.getItem("auth_permisos")?.includes("formulas.destroy");
        ...
        return (
          ...
          <Stack direction="row" spacing={2}>
            ...
            {hasDestroyPermission && (
              <Button variant="outlined" onClick={() => deleteFormula(params.id)} startIcon={<DeleteIcon />} color="warning">
                Eliminar
              </Button>
            )}
          </Stack>
      ...
      <div className={classes.tableContainer}>
        ...
        >
          {/* <DataGrid
            ...
            rows={formula}
            columns={FormulasColumns.concat(actionColumn)}
            pageSize={10}
            components={{
              Toolbar: GridToolbar,
            }}
