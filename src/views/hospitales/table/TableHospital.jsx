...
import { HospitalesColumns } from './DataHospital';
...
function TableHospital({ searchTerm, setSearchTerm }) {
  ...
  const memoizedHospitales = useMemo(() => {
    return hospital;
  }, [hospital]);
  ...
  const hasUpdatePermission = sessionStorage.getItem("auth_permisos")?.includes("hospitales.update");
  const hasDestroyPermission = sessionStorage.getItem("auth_permisos")?.includes("hospitales.destroy");
        ...
        return (
          <Stack direction="row" spacing={2}>
            ...
            {hasDestroyPermission && (
              <Button variant="outlined" onClick={() => deleteHospital(params.id)} startIcon={<DeleteIcon />} color="warning">
                Eliminar
              </Button>
            )}
          </Stack>
      ...
      <div className={classes.tableContainer}>
        ...
        >
          {/* <input
            ...
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            style={{ marginBottom: '10px' }} // Añade margen inferior para separarlo de la tabla
          /> */}
          <DataGrid
            {...memoizedHospitales}
            rows={hospital.filter((row) => {
              // Filtra las filas basadas en el término de búsqueda
              return (
                (row.cod_red?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
                (row.desc_red?.toLowerCase() || '').includes(searchTerm.toLowerCase())
              );
            })}
            columns={HospitalesColumns.concat(actionColumn)}
            pageSize={10}
            components={{
              Toolbar: GridToolbar,
            }}
            initialState={{
              ...memoizedHospitales.initialState,
