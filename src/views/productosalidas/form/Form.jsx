...
const URI1 = Api_URL + "salidaproductos/";
...
const Form = () => {
  ...
  const getSalida = async () => {
    try {
      ...
      setCantidadSalida(response.data.cantidad_salida);
      setFechaSalida(response.data.fecha_salida);
      setObsSalida(response.data.obs_salida);
      setProductoId(response.data.productos_id);
  ...
  const handleSubmit_1 = async (event) => {
    try {
      ...
      if (id) {
        const response = await axios.put(`${URI1}${id}`, {
  ...
  return (
    <Card>
      <ArgonBox p={1} mb={1} textAlign="center">
        <CardHeader
          title={id ? "ACTUALIZAR UNA SALIDA DE UN PRODUCTO" : "REGISTRAR SALIDA DE PRODUCTO"}
        />
      </ArgonBox>
        ...
        <ArgonBox component="form" role="form" onSubmit={handleSubmit_1}>
                ...
                <Item>
                  ...
                  <Autocomplete
                    ...
                    onChange={(event, newValue) => {
                      ...
                      setErrorList({ ...errorList, productos_id: "" });
                    }}
                    disablePortal
                    ...
                    options={productos}
                    getOptionLabel={(option) => option.catalogo?.nombre_producto}
                ...
                <Item>
                  ...
                  <ArgonInput
                    ...
                    onChange={(event) => {
                      ...
                      setErrorList({ ...errorList, cantidad_salida: "" });
                    }}
                  />
                </Item>
                {errorList.cantidad_salida && (
                  <span style={{ color: "red", fontSize: "14px", marginTop: "5px" }}>
                    {errorList.cantidad_salida}
                  </span>
                )}
                ...
                <Item>
                  ...
                  <ArgonInput
                    ...
                    onChange={(event) => {
                      ...
                      setErrorList({ ...errorList, fechaSalida: "" });
                    }}
                  />
                </Item>
                {errorList.fecha_salida && (
                  <span style={{ color: "red", fontSize: "14px", marginTop: "5px" }}>
                    {errorList.fecha_salida}
                  </span>
                )}
              ...
              <Grid item xs={8}>
                <Item>
                  ...
                  <ArgonInput
                    value={obsSalida}
                    onChange={(event) => {
                      setObsSalida(event.target.value);
                      setErrorList({ ...errorList, obs_salida: "" });

                    }}
                  />
                </Item>
                {errorList.obs_salida && (
                  <span style={{ color: "red", fontSize: "14px", marginTop: "5px" }}>
                    {errorList.obs_salida}
                  </span>
                )}
          ...
          <ArgonBox display="flex" justifyContent="center" mt={4} mb={2}>
            ...
            <CustomButton
              ...
              fullWidth
            >
              <Link to={"/productosalidas"} style={{ color: "white" }}>
                Cancelar
              </Link>
            </CustomButton>
          </ArgonBox>
